...
class NutrientGoalViewModel @Inject constructor(
    ...
    // called from ui when anything the user will do
    fun onEvent(event: NutrientGoalEvent) {
        when (event) {
            is NutrientGoalEvent.OnCarbRatioEnter -> {
                state = state.copy(
                    carbsRatio = filterOutDigits(event.ratio)
                ) // The copy() function is a convenient way to create a new object with the same values as the original object, except for the specified properties that are explicitly overridden.
            }
