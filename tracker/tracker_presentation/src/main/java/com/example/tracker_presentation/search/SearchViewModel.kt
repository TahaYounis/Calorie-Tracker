...
class SearchViewModel @Inject constructor(
    ...
    fun onEvent(event: SearchEvent) {
        when (event) {
            ...
            is SearchEvent.OnSearchFocusChange -> {
                state = state.copy(
                    isHintVisible = !event.isFocused && state.query.isBlank() // show hint if textField not focused and not have text in it
                )
            }
    ...
    private fun trackFood(event: SearchEvent.OnTrackFoodClick) {
        viewModelScope.launch {
            // find food item we clicked to save it in database
